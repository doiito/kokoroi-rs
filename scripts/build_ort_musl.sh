...
# stacktrace.cc includes it unconditionally on POSIX).
MUSL_STUBS_DIR="${BUILD_DIR}/musl-stubs"
mkdir -p "$MUSL_STUBS_DIR"
cat > "$MUSL_STUBS_DIR/execinfo.h" << 'EXECINFO_STUB'
#ifndef _EXECINFO_H
#define _EXECINFO_H
#ifdef __cplusplus
extern "C" {
...
cmake -S "$ORT_SRC/cmake" -B "$BUILD_DIR" \
    ...
    -DCMAKE_C_FLAGS="$ARCH_CFLAGS ${STUB_INCLUDE}" \
    -DCMAKE_CXX_FLAGS="$ARCH_CFLAGS ${STUB_INCLUDE}" \
