...
impl TTSKoko {
    ...
    pub async fn from_config(model_path: &str, voices_path: &str, cfg: InitConfig) -> Self {
        ...
        {
            if !Path::new(model_path).exists() {
                panic!("Model file not found: {}. Enable 'download' feature to auto-download.", model_path);
            }
            if !Path::new(voices_path).exists() {
                panic!("Voices file not found: {}. Enable 'download' feature to auto-download.", voices_path);
            }
        }
    ...
    fn process_internal(
        ...
        let process_one_chunk = |chunk: &str,
         ...
         -> Result<TtsOutput, Box<dyn std::error::Error>> {
            let _chunk_info = format!("Chunk: {}, ", chunk_num);
            ...
            let (mut tokens, word_map) = if _use_alignment {
    ...
    fn tokenize_with_alignment(
        ...
        // robust timestamps even when phonemizer merges words (e.g., "the model").

        // 1) Full-phrase phonemes and tokens (prosody source)
        let full_phonemes = if lan == "zh" {
            self.phonemizer.phonemize(text, lan)
        } else {
            text.to_string()
        };
        ...
        for it in &items {
            if it.len() == 1 && ".,!?:;".contains(it.chars().next().unwrap()) {
                ...
                let ph = if lan == "zh" {
                    self.phonemizer.phonemize(it, lan)
                } else {
                    it.to_string()
                };
    ...
    fn split_text_into_chunks(&self, text: &str, max_tokens: usize, lan: &str) -> Vec<String> {
        ...
        for sentence in sentences {
            ...
            let sentence_phonemes = if lan == "zh" {
                self.phonemizer.phonemize(&sentence, lan)
            } else {
                sentence.clone()
            };
            ...
            if token_count > max_tokens {
                ...
                for word in words {
                    ...
                    let test_phonemes = if lan == "zh" {
                        self.phonemizer.phonemize(&test_chunk, lan)
                    } else {
                        test_chunk.clone()
                    };
                ...
                let test_phonemes = if lan == "zh" {
                    self.phonemizer.phonemize(&test_text, lan)
                } else {
                    test_text.clone()
                };
                let test_tokens = tokenize(&test_phonemes).len();
    ...
    pub fn mix_styles(
        ...
        if !style_name.contains("+") {
            if let Some(style) = self.styles.get(style_name) {
                let max_idx = style.len().saturating_sub(1);
                let idx = tokens_len.min(max_idx);
                let styles = vec![style[idx][0].to_vec()];
                Ok(styles)
            } else {
...
impl TTSKokoParallel {
    ...
    fn get_tts_wrapper(&self, model_instance: Arc<Mutex<ort_koko::OrtKoko>>) -> TTSKoko {
        TTSKoko {
            ...
            phonemizer: self.phonemizer.clone(),
        }
    }
    ...
    /// TTS with file output for a specific model instance
    pub fn tts_with_instance(
        &self,
        opts: TTSOpts,
        model_instance: Arc<Mutex<ort_koko::OrtKoko>>,
    ) -> Result<(), Box<dyn std::error::Error>> {
