...
- 🌍 **浏览器 WASM 推理** — 支持通过 `wasm` feature 编译为 WASM，在浏览器中运行纯 Rust 推理（oxionnx 后端）或 ONNX Runtime Web
...
构建后可通过 `static/` 目录下的 `wasm_demo.html`、`browser_demo.html`、`rust_wasm_demo.html` 在浏览器中测试。
...
│   ├── rust_wasm_demo.html    # 纯 Rust WASM (oxionnx) 演示
...
- `models/onnx/model_fp16.onnx` — ONNX 模型文件
...
- [oxionnx](https://github.com/pykeio/oxionnx) — 纯 Rust ONNX 推理引擎（WASM 场景）
